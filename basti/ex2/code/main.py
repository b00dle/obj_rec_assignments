...
def compute_roundness_and_cornerness(auto_cor_mat):
    ...
    :return: 
    '''
    rows = len(auto_cor_mat)
    cols = len(auto_cor_mat[15])
    roundness = np.zeros((rows, cols))
    cornerness = np.zeros((rows, cols))
    for r in range(0, rows):
        if len(auto_cor_mat[r]) == 0:
            continue
        for c in range(0, cols):
            if len(auto_cor_mat[r][c]) == 0:
                continue
            M = np.asarray(auto_cor_mat[r][c])
            M_trace = np.trace(M)**2
            roundness[r,c] = (4 * np.linalg.det(M)) / (np.trace(M)**2)
...
def compute_auto_correlation_matrix(image_x, image_y):
    ...
    auto_cor_mat = [[] for r in range(0, image_x.shape[0])]
    for r in range(padding, image_x.shape[0] - padding):
        auto_cor_mat[r] = [[] for c in range(0, image_x.shape[1])]
